...
if (isset($_GET['id'])) {
    $courseId = $_GET['id'];
    $course = Course::getCourseById($conn, $courseId);
    $user = unserialize($_SESSION['user']);

    $isEnrolled = false;
    if ($user->getRole() == 'Student') {
        $isEnrolled = Cours_inscription::isStudentEnrolled($conn, $user->getIdUser(), $courseId);

    }

    if(isset($_POST['enrolInlCourse'])){
        $user->addEnrollemnt($conn,$courseId);
        header('Location: ../views/CourseDetails.php?id=' . $courseId);
    }

    if(isset($_POST['removeEnrollement'])){
        $user->removeEnrollemnt($conn,$courseId);
        header('Location: ../views/CourseDetails.php?id=' . $courseId);
    }
} else {
    echo "No Course ID.";
...
<body class="text-gray-800 font-inter">
    ...
    <main class="w-full md:w-[calc(100%-256px)] md:ml-64 bg-gray-200 min-h-screen transition-all main">
                    ...
                    <div class="">
                        <div class="max-w-7xl mx-auto space-y-6">
                            <div class="bg-white shadow-lg rounded-lg overflow-hidden transform ease-in-out">
                                <div class="relative">
                                    <img src="../../public/<?php echo $course['image']; ?>" alt="Course Image" class="w-full h-48 sm:h-64 md:h-72 lg:h-80 object-cover">
                                        ...
                                        <div class="space-y-2 sm:space-y-3 text-center sm:text-left">
                                            ...
                                            <p class="hidden sm:block text-white text-sm sm:text-base md:text-lg opacity-90">
                                                ...
                                                <span class="font-semibold">
                                                    <?php
                                                    $CategoryName = Category::getCategoryById($conn, $course['idCategory']);
                                                    echo $CategoryName;
                                                    ?>
                                                </span>
                                            </p>
                                            <p class="hidden sm:block text-white text-sm sm:text-base md:text-lg opacity-90">
                                                By: 
                                                <span class="font-semibold">
                                                    <?php echo Teacher::getInstructorName($conn, $course['idTeacher']); ?>
                                                </span>
                                            </p>
                                        </div>
                                        <div class="mt-4 sm:mt-6 w-full sm:w-auto">
                                        <?php if (($user->getRole() == 'Teacher' && $course['idTeacher'] == $user->getIdUser()) || $user->getRole() == 'Admin' ) { ?>
                                            <button id="viewCourseButt" class="w-full sm:w-auto bg-green-500 text-white px-6 py-3 rounded-lg shadow-lg hover:bg-green-600 focus:outline-none focus:ring-2 focus:ring-green-400 focus:ring-offset-2 flex items-center justify-center transition-all duration-300 transform hover:scale-105 active:scale-95">
                                                <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                                    <path stroke-linecap="round" stroke-linejoin="round" d="M9 5v14l11-7-11-7z" />
                                                </svg>
                                                View Course
                                            </button>
                                            <?php } elseif ($user->getRole() == 'Student') {
                                          ...
                                          <button id="viewCourseButt" class="w-full sm:w-auto bg-green-500 text-white px-6 py-3 rounded-lg shadow-lg hover:bg-green-600 focus:outline-none focus:ring-2 focus:ring-green-400 focus:ring-offset-2 flex items-center justify-center transition-all duration-300 transform hover:scale-105 active:scale-95">
                                               ...
                                               <?php }else{ ?>
                                                ...
                                                <button name="enrolInlCourse" class="w-full sm:w-auto bg-blue-500 text-white px-6 py-3 rounded-lg shadow-lg hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-400 focus:ring-offset-2 flex items-center justify-center transition-all duration-300 transform hover:scale-105 active:scale-95">
                                                <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                                <div class="col-span-2 bg-white p-6 rounded-lg shadow-lg space-y-4">
                                                ...
                                                <?php if($course['typeCours'] == "pdf"){ ?>
                                                    <div class="pdf-container bg-gray-100 flex flex-col items-center max-w-full mx-auto p-4">
                                                        ...
                                                        <div class="pagination flex justify-center items-center gap-4">
                                                            <button id="prev-page" onclick="changePage(-1)" class="text-xs sm:text-sm md:text-base">Previous</button>
                                                            <span class="text-xs sm:text-sm md:text-base">Page: <span id="page-num">1</span> / <span id="page-count">1</span></span>
                                                            <button id="next-page" onclick="changePage(1)" class="text-xs sm:text-sm md:text-base">Next</button>
                                                        </div>
                                                    </div>
                                                <?php } else { ?>
                                                    <div class="">
                                                        <div class="relative aspect-w-16 aspect-h-9">
                                                            <video class="w-full h-full object-cover" controls>
                                                                <source src="../../public/<?php echo $course['contenu']; ?>" type="video/mp4">
                                    ...
                                    <div class="space-y-2">
                                        ...
                                        <p class="text-gray-600">Type: <span class="font-medium"><?php echo $course['typeCours']; ?></span></p>
                                        <p class="text-gray-600">Category: <span class="font-medium"><?php echo $CategoryName; ?></span></p>
                                        <p class="text-gray-600">Date of Creation: <span class="font-medium"><?php echo $course['date_creation']; ?></span></p>
                                    </div>
                                    <?php if ($user->getRole() == 'Teacher' && $course['idTeacher'] == $user->getIdUser()) { ?>
                                        <form method="POST" action="">
                                            <div class="flex flex-col sm:flex-row sm:space-x-4 space-y-4 sm:space-y-0 mt-6">
                                                <button class="w-full sm:w-auto bg-blue-500 text-white px-4 py-2 rounded-lg shadow-md hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-400 flex items-center justify-center" id="editcoursee" name="editcourse">
                                                    <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                                        <path stroke-linecap="round" stroke-linejoin="round" d="M11 17.778l8.072-8.072a2 2 0 000-2.828l-2.828-2.828a2 2 0 00-2.828 0L5.344 12.122a2 2 0 00-.586 1.414V18h4.464a2 2 0 001.414-.586zM16 5l3 3" />
                                                    </svg>
                                                    Edit Course
                                                </button>
                                                <button class="w-full sm:w-auto bg-red-500 text-white px-4 py-2 rounded-lg shadow-md hover:bg-red-600 focus:outline-none focus:ring-2 focus:ring-red-400 flex items-center justify-center" name="deletecourse" onclick="return confirmDelete()">
                                                    <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2" type="submit">
                                                        <path stroke-linecap="round" stroke-linejoin="round" d="M9 2h6a2 2 0 012 2v2h3a1 1 0 011 1v1a1 1 0 01-1 1h-1v10a2 2 0 01-2 2H8a2 2 0 01-2-2V8H5a1 1 0 01-1-1V6a1 1 0 011-1h3V4a2 2 0 012-2zm0 4v10m6-10v10" />
                                                    </svg>
                                                    Delete Course
                                                </button>
                                            </div>
                                    ...
                                    <?php } elseif ($user->getRole() == 'Student') {
                                        ...
                                        <div class="flex flex-col sm:flex-row sm:space-x-4 space-y-4 sm:space-y-0 mt-6">
                                            <form action="" method="POST">
                                            <button name="removeEnrollement" class="w-full sm:w-auto bg-red-500 text-white px-4 py-2 rounded-lg shadow-md hover:bg-red-600 focus:outline-none focus:ring-2 focus:ring-red-400 flex items-center justify-center" name="enrollcourse">
                                            <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
        ...
        <form method="POST" action="addEditCourse.php" id="courseForm" enctype="multipart/form-data">
            <div id="CourseModel" class="fixed inset-0 flex justify-center items-center bg-black bg-opacity-50 z-50 hidden">
                <div id="CourseForm" class="bg-white rounded-lg w-full max-w-[60rem] sm:max-w-3/4 md:max-w-2/3 p-4 sm:p-6 shadow-lg overflow-y-auto">
                    ...
                    <div class="flex flex-col sm:flex-row space-y-4 sm:space-y-0 sm:space-x-6">
                        <div class="w-full max-w-[50rem] sm:w-2/3 space-y-4">
                            <div class="flex flex-col">
                                ...
                                <input type="text" id="Titre_Course" name="Titre_Course" value="<?php echo $course['titre']; ?>" class="mt-2 p-2 sm:p-3 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="Ex: First Principles of Machine Learning">
                            </div>
                            <div class="flex flex-col">
                                ...
                                <input type="hidden" id="Description_Course" name="Description_Course" value="<?php echo htmlspecialchars($course['description']); ?>">
                            </div>
                            <div class="flex flex-col">
                        ...
                        <div class="w-full sm:w-1/3 bg-gray-100 rounded-lg p-4 space-y-4">
                            ...
                            <div class="flex flex-col">
                                ...
                                <input type="file" id="Image_Course" name="Image_Course" accept=".jpg, .jpeg, .png" value="<?php echo $course['image']; ?>" class="mt-2 p-2 sm:p-3 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                            </div>
                            <div class="flex flex-col">
                    ...
                    <div class="mt-6 sm:mt-8 flex justify-between space-y-4 sm:space-y-0 sm:flex-row">
                        ...
                        <button type="submit" value="<?php echo $course['idCours']; ?>" name="editForm" class="bg-green-600 text-white py-2 sm:py-3 px-6 rounded-md hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-green-500">Update</button>
                    </div>
                </div>
            </div>
        </form>
    ...
    <script>
        ...
        const url = '../../<?php echo $course['contenu']; ?>'; 
